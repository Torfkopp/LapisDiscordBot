...
class JoJoWatch(Extension):
    ...
    async def jojo_function(self, ctx: SlashContext):
        await ctx.send("JoJo")
    ...
    @jojo_function.subcommand(
        sub_cmd_name="table", sub_cmd_description="Tabellarische Zusammenfassung"
    )
    ...
    @jojo_function.subcommand(
        sub_cmd_name="graph", sub_cmd_description="Graphische Zusammenfassung"
    )
    @slash_option(
        name="parts",
...
def get_info():
    ...
    gif = random.choice(
        [
            "https://c.tenor.com/L7rMSFzEJq4AAAAC/tenor.gif",
            "https://c.tenor.com/i-yPjO4lTG0AAAAd/tenor.gif",
...
def get_graph(wanted_parts, compare):
    jojo_dict = []
    # Parse CSV
    with open("variable/jojo-history.csv", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            jojo_dict.append(
                {
...
def get_comparison(dates, parts, show_parts, part_color_map):
    ...
    for part, (days_since_start, episode_count) in part_progression.items():
        ax.plot(
            days_since_start, episode_count, color=part_color_map.get(part, "white"), label=part
        )
...
def get_timeline(dates, parts, show_parts, part_color_map):
    ...
    for idx in range(len(filtered_part_indices) - 1):
        ...
        ax.plot(
            ...
            filtered_counts[start:end],
            color=part_color_map.get(part, "white"),
