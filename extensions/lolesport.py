...
def create_schedule():
    ...
    if isinstance(events, interactions.Embed):
    ...
    last_game_time = list(start_times)[-1]
    last_game_time += list(events)[-1]["match"]["strategy"]["count"] * datetime.timedelta(minutes=40)
...
def get_live():
    ...
    for event in events:
        ...
        time = datetime.datetime.fromisoformat(event["startTime"].replace("Z", "+00:00"))
        time = time.astimezone(pytz.timezone("Europe/Berlin")).replace(tzinfo=None)
        if not time.date() == today:
            continue

        league = event["league"]["name"]
        if embeds.get(league) is None:
            embeds[league] = interactions.Embed(title=league, color=COLOUR)
        embed = embeds[league]
        ...
        if event["state"] == "completed":
            ...
            time = f"Startet um: {datetime.datetime.strftime(time, '%d.%m %H:%M')}"
        ...
        embed.add_field(
            name=f"({record1}) {team1['name']} {wins1}:{wins2} {team2['name']} ({record2})",
            value=f"{time}, {bo_format}",
            inline=True,
        )  # See how inlining looks
...
def league_slash_option():
    def wrapper(func):
        ...
        for key in LEAGUE_DICT:
            choices.append(SlashCommandChoice(name=key, value=LEAGUE_DICT.get(key)))
        return slash_option(
            name="league",
...
class LoLesports(Extension):
    ...
    async def lol_function(self, ctx: SlashContext):
        await ctx.send("LoL")

    @lol_function.subcommand(
        sub_cmd_name="results", sub_cmd_description="Die Ergebnisse der letzten Matches"
    )
    @league_slash_option()
...
def get_upcoming(league):
    ...
    for event in reversed(events):
        if event["state"] != "unstarted":
            break  # after seeing a "not unstarted" game, every game will be completed
    ...
    for event in upcoming:
        ...
        team2 = event["match"]["teams"][1]
        record1 = (
            f"{team1['record']['wins']}-{team1['record']['losses']}"
            if team1["record"] is not None
            else ""
        )
        record2 = (
            f"{team2['record']['wins']}-{team2['record']['losses']}"
            if team2["record"] is not None
            else ""
        )
        ...
        bo_format = f"{(event['match']['strategy']['type']).capitalize()} {event['match']['strategy']['count']}"
        embed.add_field(
            name=f"({record1}) {team1['name']} gegen {team2['name']} ({record2})",
            value=f"{start_time} | {league} {block} {bo_format}",
        )

    embed.set_footer(
        f"Für mehr: https://lolesports.com/schedule?leagues={events[0]['league']['slug']}"
    )
...
def get_results(league):
    ...
    for event in completed:
        ...
        embed.add_field(
            name=f"({record1}) {team1['name']} {wins1}:{wins2} {team2['name']} ({record2})",
            value=f"{start_time} | {league} {block}",
            inline=True,
        )
                ...
                for columns in section["columns"]:
                    for cell in columns["cells"]:
                        ...
                        for match in cell["matches"]:
                            ...
                            team2 = teams[1]["code"]
                            try:
                                wins1 = teams[0]["result"]["gameWins"]
                            except TypeError:
                                wins1 = 0
                            try:
                                wins2 = teams[1]["result"]["gameWins"]
                            except TypeError:
                                wins2 = 0
