...
async def function(ctx, entry_function, *args):
    ...
    entry_function(*args)
    try:
        await Tierlist.message.edit(embed=Tierlist.embed)
    except AttributeError:
        pass


async def on_message_delete(msg):
    with open("variable/tierlist.json", encoding="utf-8") as f:
        j = json.load(f)
    x = j.pop(str(msg.id), None)
    if x:
        with open("variable/tierlist.json", "w", encoding="utf-8") as f:
...
class Tierlist(Extension):
    ...
    async def tierlist_function(
        self, ctx: SlashContext, name, tiers: str = "S", description: str = ""
    ):
    ...
    @prefixed_command()
    async def c(self, ctx: PrefixedContext, arg1, arg2):
        await function(ctx, create_entry, arg1, arg2)

    @prefixed_command()
    async def d(self, ctx: PrefixedContext, arg1, arg2):
        await function(ctx, delete_entry, arg1, arg2)

    @prefixed_command()
    async def m(self, ctx: PrefixedContext, arg1, arg2, arg3):
        await function(ctx, move_entry, arg1, arg2, arg3)

    # Just alternative naming
    @prefixed_command()
    async def create(self, ctx: PrefixedContext, arg1, arg2):
        await function(ctx, create_entry, arg1, arg2)

    @prefixed_command()
    async def delete(self, ctx: PrefixedContext, arg1, arg2):
        await function(ctx, delete_entry, arg1, arg2)

    @prefixed_command()
    async def move(self, ctx: PrefixedContext, arg1, arg2, arg3):
        await function(ctx, move_entry, arg1, arg2, arg3)
...
def create_tierlist(theme, tiers, description):
    ...
    embed = interactions.Embed(
        title=f"{theme} Tierlist",
        description=f"Ordnung: {description}",
        color=util.Colour.TIERLIST.value,
    )
    embed.set_footer(
        "@Lapis + (c/create {Tier} {Name} | d/delete {Tier} {Name} | m/move {Tier1} {Tier2} {Name})"
    )
...
def create_entry(tier, name):
    ...
    for field in tier_field:
        field.value = f"{field.value} {name},"
...
def delete_entry(tier, name):
    ...
    tier_field = [field for field in Tierlist.embed.fields if field.name == tier]
    for field in tier_field:
        field.value = field.value.replace(f"{name},", "")
