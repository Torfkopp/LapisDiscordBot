...
def get_current():
    ...
    if len(event_schedule) == 0:  # If no remaining races, set next_event to last race
        next_event = fastf1.get_event(
            util.CURRENT_F1_SEASON, len(fastf1.get_event_schedule(util.CURRENT_F1_SEASON)) - 1
        )
    else:
        next_event = event_schedule.iloc[0]
        if next_event["RoundNumber"] == 0:
            next_event = event_schedule.iloc[1]
    ...
    for i in range(len(session_list)):  # A session should be finished 1.5 hours after the start
        if session_list[i] + datetime.timedelta(hours=1.5) <= date_today:
            latest_finished_session = i + 1
...
def _handle_event_change_during_weekend(date_today, next_event):
    ...
    """
    if next_event["RoundNumber"] == 1:
        return next_event  # FastF1 can't handle RoundNumber 0
    if date_today.weekday() > 4 and next_event["EventDate"] > date_today + datetime.timedelta(
        days=3
    ):
        temp_event = fastf1.get_event(date_today.year, next_event["RoundNumber"] - 1)
        if temp_event["Session5Date"].astimezone(pytz.timezone("Europe/Berlin")).replace(
            tzinfo=None
        ) < date_today + datetime.timedelta(days=3):
            next_event = temp_event
    return next_event
...
def f1_info():
    ...
    if len(event_schedule) == 0:
        return embed
    first_session_date = (
        event_schedule.iloc[0]["Session1Date"]
    ...
    if (date_today.weekday() == 3) and (
        (first_session_date.date() - date_today.date()) == datetime.timedelta(days=1)
    ):
...
def create_schedule():
    ...
    session_map = {
        next_event["Session1Date"]
        .astimezone(pytz.timezone("Europe/Berlin"))
        .replace(tzinfo=None): next_event["Session1"],
        next_event["Session2Date"]
        .astimezone(pytz.timezone("Europe/Berlin"))
        .replace(tzinfo=None): next_event["Session2"],
        next_event["Session3Date"]
        .astimezone(pytz.timezone("Europe/Berlin"))
        .replace(tzinfo=None): next_event["Session3"],
        next_event["Session4Date"]
        .astimezone(pytz.timezone("Europe/Berlin"))
        .replace(tzinfo=None): next_event["Session4"],
        next_event["Session5Date"]
        .astimezone(pytz.timezone("Europe/Berlin"))
        .replace(tzinfo=None): next_event["Session5"],
    }

    return {
        date: session_map.get(date)
        for date in session_map.keys()
...
def auto_result(result_only: bool):
    ...
    if result_only and response["period"] != "FULL_TIME":
        return None, True

    try:
        result = util.uwuify_by_chance(_make_result(response))
    except:
        result = None  # If something doesn't work with making the result, treat it the same as an ended session

    if response["period"] == "FULL_TIME" or not result:
        result_url = None  # Reset the result_url if a session ends
