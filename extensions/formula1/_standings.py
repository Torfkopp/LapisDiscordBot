    ...
    for rnd, race in races["raceName"].items():
        ...
        if rnd == 0:  # If it is the first iteration, fill driver and team map
            for i in range(temp.index.max()):
                ...
                if not (status == "Finished" or status.startswith("+")):
                    position = "DNF"
            ...
            for i in range(temp.index.max()):
                ...
                if driver_code in result_map:  # Check necessary to avoid AttributeError
                    ...
                    driver_map[driver_code] = (
                        f"{temp.at[i, 'givenName']} {temp.at[i, 'familyName']}"
                    )
...
def average_position(year, session):
    ...
    for team in team_order:
        team_colours.append(fastf1.plotting.team_color(team))
...
def compare_positions(driver1_positions, driver2_positions):
    ...
    for race in driver1_positions.keys() & driver2_positions.keys():
        if driver1_positions.get(race) == "DNF" or driver2_positions.get(race) == "DNF":
            continue
...
def h2h(year, session):
    ...
    for team in race_positions[2]:
        ...
        if len(drivers) < 2:
            ...
            driver1_positions, driver2_positions = (
                race_positions[0].get(drivers[0]),
                race_positions[0].get(drivers[1]),
            )
            driver1_points, driver2_points = compare_positions(driver1_positions, driver2_positions)
            result_string += add_to_string(
                team,
                race_positions[1].get(drivers[0]),
                driver1_points,
                driver2_points,
                race_positions[1].get(drivers[1]),
            )
        elif len(drivers) == 3:
            ...
            driver1a_points, driver2a_points = compare_positions(
                driver1_positions, driver2_positions
            )
            # driver 3 vs driver 1/2 to the end
            if len(driver1_positions) > len(driver2_positions):
                driver_1or2 = drivers[0]
                driver1b_points, driver2b_points = compare_positions(
                    driver1_positions, driver3_positions
                )
            else:
                driver_1or2 = drivers[1]
                driver1b_points, driver2b_points = compare_positions(
                    driver2_positions, driver3_positions
                )
            result_string += add_to_string(
                team,
                race_positions[1].get(drivers[0]),
            ...
            result_string += add_to_string(
                " ",
                race_positions[1].get(driver_1or2),
                driver1b_points,
                driver2b_points,
                race_positions[1].get(drivers[2]),
            )
        else:
            driver1_positions, driver2_positions = (
                race_positions[0].get(drivers[0]),
...
def heatmap(year):
    ...
    fig.update_yaxes(
        showgrid=True, gridwidth=1, gridcolor="White", showline=False, tickson="boundaries"
...
def whocanwin():
    ...
    for i, _ in enumerate(driver_standings.iterrows()):
        ...
        embed.add_field(
            name=f"{driver['position']}: {driver['givenName'] + ' ' + driver['familyName']}, "
            f"Momentane Punkte: {driver['points']}",
            value=f"Maximal erreichbare Punkte: {driver_max_points}, Can win: {can_win}",
        )
