...
def overview_fastest_laps(year, gp, session):
    ...
    for drv in drivers:
        ...
        if drvs_fastest_lap.isnull().sum() == len(drvs_fastest_lap.values):
            continue
    ...
    max_width = 0
    for bar in bars:
        if bar.get_width() > max_width:
            max_width = bar.get_width()
...
def compare_laps(year, gp, session, driver1, driver2):
    ...
    try:
        ...
        dr1_color = util.random_colour_generator()
    try:
        dr2_color = fastf1.plotting.driver_color(driver2)
    except KeyError:
        dr2_color = util.random_colour_generator()
    ...
    ax.plot(
        ...
        dr1_tel["Speed"],
        color=dr1_color,
        label=f"{driver1} {dr1_lap_time: >10}",
    )
    ax.plot(
        dr2_tel["Distance"],
        dr2_tel["Speed"],
        color=dr2_color,
        label=f"{driver2} {dr2_lap_time: >10}",
    )
...
def telemetry(year, gp, session, driver1, driver2):
    ...
    if year == util.CURRENT_F1_SEASON:
        try:
            ...
            d1_color = util.random_colour_generator()
        try:
            d2_color = fastf1.plotting.driver_color(d2_name)
        except KeyError:
            d2_color = util.random_colour_generator()
    else:
    ...
    ax[1].annotate(
        throttle_string, xy=(1.0, -0.4), xycoords="axes fraction", ha="right", va="center"
    )
...
def track_dominance(year, gp, session, driver1, driver2):
    ...
    combined_data = pd.concat(
        [
            d1_telemetry_data[["X", "Y", "Minisector", "Minisector_Speed", "Driver_Color"]],
    ...
    filtered_df = filtered_df.loc[
        filtered_df.groupby(filtered_df.index)["Minisector_Speed"].idxmax()
    ]
    ...
    plt.title(
        ...
        f"Track Dominance on Fastest Lap",
        weight="bold",
    )
    plt.grid(visible=False, which="both")
    # set up legend
