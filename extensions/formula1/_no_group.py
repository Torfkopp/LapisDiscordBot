...
def result(year, gp, session):
    ...
    if sess.name == "Qualifying":
        ...
        def format_time(td):
            return (
                f"{td.seconds // 60}:{(td.seconds % 60 + td.microseconds / 1e6):06.3f}"
                if not pd.isnull(td)
                else ""
            )
        ...
        def format_time(td):
            ...
            if td >= pd.Timedelta(hours=1):
                return f"{c.hours}:{c.minutes:02d}:{c.seconds:02d}.{c.milliseconds:03d}"
...
def next_race():
    ...
    embed = interactions.Embed(
        title=f"Race Schedule {event_name}",
        color=COLOUR,
    ...
    for i in range(len(session_list)):
        ...
        sessions += (
            (next_event[f"Session{i + 1}"] + ":").ljust(16)
            + session[0].center(12)
            + session[1].rjust(12)
        ) + "\n"
    ...
    try:
        ...
        if not response.ok:
            ...
            if circuit == "Azerbaijan":
                circuit = "Baku"
            elif circuit == "United States":
                circuit = "USA"
            else:
                circuit = circuit.replace(" ", "_").replace("-", "_")
