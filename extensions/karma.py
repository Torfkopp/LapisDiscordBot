...
class Karma(Extension):
    ...
    async def karma_function(self, ctx: SlashContext):
        await ctx.send("Karma")

    @karma_function.subcommand(
        sub_cmd_name="author_ranking", sub_cmd_description="Ein Ranking aller Komödianten hier"
    )
...
async def on_message(msg):
    if str(msg.channel.id) != util.COMEDY_CHANNEL_ID:
        return
    regex = re.search(
        r"(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,})",
        msg.content,
    )  # Allow Links
    if not (msg.attachments or msg.embeds or msg.interaction_metadata or regex):
        return  # Return if not pic or embed
...
async def on_reaction(reac):
    if reac.emoji.id not in [UPVOTE_ID, MEH_ID, DOWNVOTE_ID]:
        return
    ...
    if (author := str(reac.message.author)) == "Lapis#7072":
        try:
            author = "@" + reac.message.interaction_metadata.user.username
        except:
            pass  # Author is reac.message.author
    ...
    if author != (reacter := str(reac.author)) and reacter != "Lapis#7072":
        if reac.emoji.id == UPVOTE_ID:
            upvoters.add(reacter)
        elif reac.emoji.id == MEH_ID:
            mehvoters.add(reacter)
        elif reac.emoji.id == DOWNVOTE_ID:
            downvoters.add(reacter)

    up, meh, down = (
        ", ".join(filter(None, upvoters)),
        ", ".join(filter(None, mehvoters)),
        ", ".join(filter(None, downvoters)),
    )
    if post:
        cur.execute(
            "UPDATE posts SET upvoters = ?, mehvoters = ?, downvoters = ? WHERE id = ?",
            [up, meh, down, msg_id],
        )
    else:
        cur.execute(
            "INSERT OR IGNORE INTO posts VALUES(?, ?, ?, ?, ?)", [author, msg_id, up, meh, down]
        )
...
async def on_reaction_remove(reac):
    ...
    if reac.emoji.id == UPVOTE_ID:
        upvoters.discard(reacter)
    elif reac.emoji.id == MEH_ID:
        mehvoters.discard(reacter)
    elif reac.emoji.id == DOWNVOTE_ID:
        downvoters.discard(reacter)
...
def get_author_ranking():
    class User:
        ...
        def calc_karma(self):
            self.karma = (
                self.posts * POST_KARMA
                + self.votings * VOTING_KARMA
                + self.upvotes * UPVOTE_KARMA
                + self.mehvotes * MEHVOTE_KARMA
                + self.downvotes * DOWNVOTE_KARMA
            )
            return self.karma
    ...
    user_set, users = set(), {}
    for author, up, meh, down in cur.execute(
        "SELECT author, upvoters, mehvoters, downvoters FROM posts"
    ).fetchall():
        user_set.update(
            [author]
    ...
    for post in posts:
        up, meh, down = (
            list(filter(None, post[2].split(", "))),
            list(filter(None, post[3].split(", "))),
            list(filter(None, post[4].split(", "))),
        )
        author = users[post[0]]
        author.posts += 1
...
def get_post_ranking(reverse):
    ...
    for p in posts:
        up, meh, down = (
            len(list(filter(None, p[2].split(", ")))),
            len(list(filter(None, p[3].split(", ")))),
            len(list(filter(None, p[4].split(", ")))),
        )
        sorted_posts.append((p[0], p[1], up, meh, down))

    if not reverse:
        sorted_posts = sorted(sorted_posts, reverse=True, key=lambda x: (x[2], x[3]))
    else:
        sorted_posts = sorted(sorted_posts, reverse=True, key=lambda x: (x[4], -x[3]))

    for i, p in enumerate(sorted_posts[:10]):
        link = "https://discord.com/channels/" + "/".join([server_id, channel_id, p[1]])
        name = (
            f"{i + 1}. {p[0]} {PartialEmoji(id=UPVOTE_ID)} {p[2]} "
...
def get_karma_graph():
    ...
    for post in posts:
        ...
        if len(up) >= LIKES_FOR_BONUS:
            users[post[0]][-1] += BONUS
        if len(down) >= LIKES_FOR_BONUS:
            users[post[0]][-1] += PENALTY
        for u in set(up + meh + down):
            users[u][-1] += VOTING_KARMA
        for u in users:
            users[u].append(users[u][-1])
