...
async def on_hangman_component(event: Component):
    ctx = event.ctx
    if not ctx.custom_id.startswith("hangman"):
        return
...
class HangmanClass:
    ...
    def build_censored_word(self, letter=None):
        ...
        if letter in self.word_to_guess:
            ...
            for i in range(len(word)):
                if i in indices:
                    word[i] = word[i].replace("_", letter)
            word = " ".join(word)
            self.word = word
            return True
        return False
    ...
    def build_embed(self, win, loss):
        ...
        embed.add_field(
            "An diesem Galgenmännchen beteiligte Personen:", ", ".join(self.participants)
        )
...
class Hangman(Extension):
    ...
    async def hangman_function(
        self, ctx: SlashContext, language: bool = True, difficulty: str = ""
    ):
...
def get_word(in_german, difficulty_option):
    ...
    difficulty = (
        random.choice(["easy", "normal", "hard"]) if difficulty_option == "" else difficulty_option
    )
...
def build_stage(current_stage):
    ...
    with open("resources/hangman.json", encoding="utf-8") as f:
        graphic = json.load(f)
    stage = "```\n"
