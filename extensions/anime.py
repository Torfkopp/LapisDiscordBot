...
class Anime(Extension):
    ...
    async def anime_function(self, ctx: SlashContext):
        await ctx.send("Anime")

    @anime_function.subcommand(
        sub_cmd_name="action", sub_cmd_description="Mach was Animeliches zu jemanden"
    )
    @slash_option(
        name="action",
        description="Aktion",
    ...
    @anime_function.subcommand(
        sub_cmd_name="reaction", sub_cmd_description="Zufällige Anime Bewegtbildreaktion"
    )
    @slash_option(
        name="theme_1",
