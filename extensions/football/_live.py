...
COMPETITION_LIST = (
    UNFILTERED_COMPETITIONS + FILTERED_COMPETITIONS
)  # List of all Leagues with at least some interest in
...
def match_interested_in(match):
    ...
    if match["competition"]["name"] in UNFILTERED_COMPETITIONS:
        return True
...
def create_schedule():
    ...
    for league in data:
        ...
        if (x := league["matches"][0]["competition"]["name"]) not in COMPETITION_LIST and not any(
            keyword in x for keyword in KEYWORDS
        ):
            continue
        # Add match times to set
        for match in league["matches"]:
            if not match_interested_in(match):
                continue
            time = datetime.datetime.fromisoformat(
                match["scheduledStartTime"].replace("Z", "+00:00")
            )
...
def get_live():
    ...
    for league in data:
        ...
        for match in league["matches"]:
            ...
            if "matchTime" in match["matchInfo"]:
                game.minute = match["matchInfo"]["matchTime"]
            if match["period"] == "FULL_TIME":
                game.done = True
            ...
            if match["isLive"] or (
                ...
                < (datetime.datetime.now(pytz.utc) - game.start_time)
...
def build_embed(league_name, round_title):
    ...
    for m in league_matches[league_name]:
        ...
        penalty_score = (
            f" {game.penalty_score[0]}:{game.penalty_score[1]} nE" if game.penalty_score else ""
        )
        ...
        if game.minute < 0 and not game.done:
            ...
            value = "END " if game.done else f"{str(game.minute).zfill(2)}' "

        goals_home, goals_away = "", ""

        for g in game.goals1:
            goals_home += goals(g, goals_home)
        for g in game.goals2:
            goals_away += goals(g, goals_away)

        if not goals_home == "":
            goals_home = game.team1_short + ":" + goals_home
        if not goals_away == "":
            goals_away = game.team2_short + ":" + goals_away

        if goals_home == "" or goals_away == "":
            value += goals_home + goals_away
        else:
            value += goals_home + "\n" + " " * 4 + goals_away
