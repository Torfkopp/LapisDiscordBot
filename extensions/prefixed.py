...
class Prefixed(Extension):
    @prefixed_command()
    async def hello(self, ctx: PrefixedContext):
        await ctx.reply(embed=hello())

    @prefixed_command()
    async def hallo(self, ctx: PrefixedContext):
        await ctx.reply(embed=hello())

    @prefixed_command()
    async def moin(self, ctx: PrefixedContext):
        await ctx.reply(embed=hello())

    @prefixed_command()
    async def danke(self, ctx: PrefixedContext):
        await ctx.reply(embed=thanks())

    @prefixed_command()
    async def thanks(self, ctx: PrefixedContext):
        await ctx.reply(embed=thanks())

    @prefixed_command()
    async def goodbye(self, ctx: PrefixedContext):
        """Shuts down the bot"""
        if ctx.author_id not in self.bot.owner_ids:
            return
        ...
        await self.bot.change_presence(
            ...
            activity=discord.activity.Activity.create(
                name="zu, dass sie herunterfährt", type=discord.activity.ActivityType.WATCHING
            ),
        )
    ...
    async def update(self, ctx: PrefixedContext):
        ...
        await self.bot.change_presence(
            status=discord.Status.DND,
            activity=discord.activity.Activity.create(
                name="Updates auf", type=discord.activity.ActivityType.GAME
            ),
        )
    ...
    async def say(self, ctx: PrefixedContext, channel="", text=""):
        ...
        match channel:
            case "sport":
                channel = util.SPORTS_CHANNEL_ID
            case "comedy":
                channel = util.COMEDY_CHANNEL_ID
            case "mod":
                channel = util.MODERATOREN_CHANNEL_ID
            case "stammrunde":
                channel = util.STAMMRUNDEN_CHANNEL_ID
            case _:
                channel = util.LABAR_CHANNEL_ID
