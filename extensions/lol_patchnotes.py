...
def update():
    ...
    if LoLPatchnotes.url and LoLPatchnotes.url != url:
        new_patchnotes = True
...
class LoLPatchnotes(Extension):
    ...
    async def patch_function(self, ctx: SlashContext):
        await ctx.send("Patch")

    @patch_function.subcommand(
        sub_cmd_name="image", sub_cmd_description="Erhalte das Patchzusammenfassungsbild"
    )
    async def image_function(self, ctx: SlashContext):
        await ctx.send(embed=get_patch_image())

    @patch_function.subcommand(
        sub_cmd_name="summary",
    ...
    @patch_function.subcommand(
        sub_cmd_name="details", sub_cmd_description="Erhalte die Änderungen des Patches im Details"
    )
    @slash_option(
        name="type_filter",
    ...
    async def details_function(
        self, ctx: SlashContext, type_filter: int = 0, name_filter: str = ""
    ):
...
def get_patch_summaries():
    ...
    embed = _make_embed()
    if len(champ_names) != len(champ_summaries):
        embed.add_field(name=champ_names.pop(0).text, value="Neuer Champ")
...
def get_midpatch_update():
    ...
    soup = LoLPatchnotes.soup
    midpatch = soup.find("h2", id="patch-mid-patch-updates") or soup.find(
        "h2", id="patch-midpatch-updates"
    )
    if midpatch is None:
        return None
    embed = _make_embed()
...
def _is_not_in_filter(name, name_filter):
    ...
    for filter_name in name_filter:
        if filter_name in name.lower().replace("’", ""):
            is_in = False
...
def _get_champion_embeds(champion_changes, name_filter):
    ...
    for champ_change in champion_changes:
        ...
        if _is_not_in_filter(name, name_filter):
            continue
        change_names = champ_change.find_all("h4")
        change_numbers = champ_change.find_all("ul")
        change_text = ""
        for j in range(len(change_names)):
            change_text += f"- {change_names[j].text}: {change_numbers[j].text}"
        change_text = change_text.replace("Fähigkeitsstärke", "AP").replace("Angriffsschaden", "AD")
            ...
            for j in range(len(change_names)):
                ...
                if len(numbers) > 1024:
                    ...
                    for li in change_numbers[j].find_all("li"):
                        li_text = li.text
                        li_text = li_text.replace("Fähigkeitsstärke", "AP").replace(
                            "Angriffsschaden", "AD"
                        )
                        if len(change_text) + len(li.text) > 1024:
                            embed_fields.append(
                                interactions.EmbedField(name=name, value=change_text)
                            )
                            change_text = f"- {change_names[j].text}: "
                        change_text += li_text
                    continue
        ...
        if change_text == "":
            change_text = f"“{champ_change.find('blockquote').text.strip()}”"
        embed_fields.append(interactions.EmbedField(name=name, value=change_text))
...
def _get_system_embeds(system_changes, name_filter):
    ...
    for system_change in system_changes:
        ...
        if not title:
            if system_change.find("p"):
                name = system_change.find("p").text
            elif system_change.find("h4"):
                name = system_change.find("h4").text
            else:
                name = system_change.parent.find_previous_sibling("header").find("h2").text
            if _is_not_in_filter(name, name_filter):
                continue
            value = ""
            for li in system_change.find_all("li"):
                ...
                value += f"- {li.text}"
            if value == "":
                value = system_change.find("blockquote").text
            embed_fields.append(interactions.EmbedField(name=name, value=value))
            continue
        name = title.text
        if _is_not_in_filter(name, name_filter):
...
def get_patch_details(type_filter, name_filter):
    ...
    for change in changes:
        category_id = change.parent.find_previous_sibling("header", class_="header-primary").find(
            "h2"
        )["id"]
        if category_id == "patch-champions":
            champion_changes.append(change)
        elif category_id in ["patch-items", "patch-runes"]:
            system_changes.append(change)
        else:
            system_changes.append(change)  # other_changes.append(change)
    ...
    def make_embeds(fields, text):
        ...
        for j in range(page_amount):
            ...
            while field_counter < len(fields):
                ...
                field_counter += 1
                if sum(len(field.value) for field in embed.fields) > 4000:
                    break
            result.append(embed)
        return result
    ...
    if midpatch_update:
        embeds.append(midpatch_update)
    if type_filter < 2:  # true for 0 and 1 (all and champions only)
        embeds += make_embeds(
            _get_champion_embeds(champion_changes, name_filter), "Championänderungen"
        )
    ...
    if util.get_if_uwuify():
        for i in range(len(embeds)):
            embeds[i] = util.uwuify_embed(embeds[i])
