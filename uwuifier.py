...
def UwUifyWords(wowd):
    ...
    for key in uwu_dic:
        wowd = wowd.replace(key, uwu_dic.get(key))
    ...
    for key in uwu_speciaw_dic:
        wowd = wowd.replace(key, uwu_speciaw_dic.get(key))
...
def UwUifySpaces(msg):
    ...
    for wowd in msg.split(" "):
        ...
        if 0 <= rnd_pewcent <= FACES_CHANCE:
            wowd += " " + random.choice(FACES_WIST)

        # Add  wandom action aftew de wowds
        elif FACES_CHANCE < rnd_pewcent <= (FACES_CHANCE + ACTION_CHANCE):
            wowd += " " + random.choice(ACTION_WIST)
