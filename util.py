...
# Other settings
CURRENT_F1_SEASON = (
    now.year if (now := datetime.datetime.now()).month > 1 else now.year - 1
)  # F1 Season end of Feb
...
def get_gif(term: str):
    ...
    with open("resources/gifs.json") as g:
        gif_db = json.load(g)

    try:
        gifs = gif_db[term]
    except KeyError:
        return gif_db["standard"]

    if isinstance(gifs, dict):
        return random.choice(list(gifs.values()))
    return gifs
...
def get_error_embed(term: str, add_text: str | list[str] = ""):
    ...
    match term:
        ...
        case "wrong_channel":
            ...
            embed.description = (
                "Die Antwort auf diesen Befehl kann ich dir leider "
        ...
        case "limit_reached":
            ...
            embed.description = (
                "Die angefragte API wurde leider in kürzester Zeit mit "
        ...
        case "faulty_value":
            ...
            embed.description = (
                "Deine eingegeben Werte stehen nicht in meiner Liste <(＿　＿)>. "
        ...
        case "api_down":
            ...
            embed.description = (
                "Die angefragte API ist momentan down und ich kann nix dagegen tun. （；´д｀）ゞ"
            )
            embed.set_image(url=get_gif(term))
        case "too_big":
            embed.title = "Too big"
...
def message_sent(message: str):
    ...
    with open("variable/message_tracker.json") as message_tracker:
        trackers = json.load(message_tracker)
    if trackers[message]:
        return True
    else:
        trackers[message] = True
        with open("variable/message_tracker.json", "w") as message_tracker:
            json.dump(trackers, message_tracker)
        return False
...
def day_counter():
    ...
    with open("variable/day_counter.json") as dc:
        counter_file = json.load(dc)
    today = datetime.datetime.now().date()
    if today == datetime.date.fromisoformat(counter_file["last_updated"]):
        return True, 0
    counter_file["last_updated"] = str(today)
    counter_file["day_counter"] += 1
    with open("variable/day_counter.json", "w") as dc:
        json.dump(counter_file, dc)
...
def uwuify_by_chance(obj):
    ...
    if not random.randint(0, 100) < UWUCHANCE:
        return obj
    if isinstance(obj, interactions.Embed):
        return uwuify_embed(obj)
...
def uwuify_embed(embed):
    ...
    if embed.description is not None:
        embed.description = uwuifier.UwUify(embed.description)
